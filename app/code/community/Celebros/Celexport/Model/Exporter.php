...
class Celebros_Celexport_Model_Exporter
{
    ...
    public function isStoreExportEnabled($store_id)
    {
        ...
        if (!isset($this->_store_export_status)) {
            ...
            foreach (Mage::app()->getStores() as $store) {
                $this->_store_export_status[$store->getId()] = FALSE;
                ...
                if ($store->getConfig('celexport/export_settings/export_enabled')) {
                    ...
                    if (!$this->isWebRun) {
                        ...
                        if ($store_view == 'export') {
                            $this->_store_export_status[$store->getId()] = TRUE;
                            continue;
                        }
                        
                        //This checks whether cron is enabled for this store view.
                        $cron_enabled = Mage::getStoreConfigFlag('celexport/export_settings/cron_enabled', $store->getId());
                        if ($cron_enabled) {
                            ...
                            if ((int)$store_view == $store->getId()) {
                                ...
                                $expected = 'crontab/jobs/celexport_export_' . $store->getId() . '/schedule/cron_expr';
                            ...
                            /*
                            $expr = Mage::getStoreConfig('celexport/export_settings/cron_expr', $store->getId());
    ...
    {
        $dir = Mage::app()->getStore(0)->getConfig('celexport/export_settings/path');
        $files = scandir($dir);
        foreach ($files as $file) {
    ...
    public function export_config($store_id)
    {
        ...
        if($this->_fDel==='\t') $this->_fDel=chr(9);
