...
class Celebros_Celexport_Model_ObserverLarge extends Celebros_Celexport_Model_Exporter
{
    ...
    {
        $collection = Mage::getModel('catalog/product')->setStoreId($store_id)->getCollection();
        $collection->addAttributeToFilter($attribute_code , $attribute_value);
    ...
    protected function export_products($sql, $fileName)
    {
        ...
        if (!Mage::getStoreConfig('celexport/advanced/single_process')) {
            ...
            foreach ($chunks as $chunk) {
                ...
                $pids[$i] = (int) shell_exec('nohup php ' . Mage::getModuleDir('', 'Celebros_Celexport') . '/Model/processChunkCol.php ' . $i . ' ' . $this->_fStore_id . ' ' . Mage::getBaseDir() . DS . 'shell ' . $this->_exportProcessId . ' > /dev/null & echo $!');
                if (!$pids[$i]) {
